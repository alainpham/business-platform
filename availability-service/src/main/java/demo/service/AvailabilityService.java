...
import demo.repo.AvailablilityRepository;
...
public class AvailabilityService {
    ...
    public ResponseEntity<Availability> checkAvailability(@RequestBody Availability input){
        ...
        if (avail.size() > 0 ){
            /* dates are already partially booked*/
            logger.info("dates are already partially booked");
            input = avail.get(0);
        }else{
            logger.info("dates are free !!");
    ...
    //     logger.info("reset avail DB as scheduled");
    //     availablilityRepository.deleteAll();
    // }
